...
async function GetSpecificUserProfile(userId: string) {
	const jtwToken = JWTStorage.getJWT();
	try {
		const res = await axios.get(`${AUTH_CONTROLLER_URL}/profile/${userId}`, {
			headers: {
				Authorization: `Bearer ${jtwToken?.token}`,
			},
		});
...
export type AuthServiceType = {
	...
	GetSpecificUserProfile: (userId: string) => Promise<Profile|null> 
	UpdateProfile: (
